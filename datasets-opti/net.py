...
#pretrained = BertModel.from_pretrained(r'D:\study\computerStudy\personcode\jukeAI\datasets-opti\model\bert-base-chinese\models--bert-base-chinese\snapshots\c30a6ed22ab4564dc1e3b2ecbf6e766b0611a33f').to(DEVICE)
#pretrained.embeddings.position_embeddings = torch.nn.Embedding(1024, 768).to(DEVICE)
config = BertConfig.from_pretrained(r'D:\study\computerStudy\personcode\jukeAI\datasets-opti\model\bert-base-chinese\models--bert-base-chinese\snapshots\c30a6ed22ab4564dc1e3b2ecbf6e766b0611a33f')
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.fc = torch.nn.Linear(768, 10)  # 输入维度为768，输出维度为10,多分类任务，用于多分类评价等
    def forward(self, input_ids, attention_mask,token_type_ids):
        ...
        #由于模型的配置参数发生了变化，所以需要对原有模型参数进行重新学习训练，不能再冻结BERT的参数。这里就相当于是全量微调
        out = pretrained(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)
