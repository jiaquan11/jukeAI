...
out = token.batch_encode_plus(
    ...
    return_tensors=None,
    #返回attention_mask
    return_attention_mask=True,#返回attention_mask，用于区分padding和非padding
...
#token_type_ids第一个句子和特殊符号的位置是0，第二个句子的位置是1  ()只针对上下文编码(类似循环神经网络),当前transform大模型不适用(因为transform有自注意力机制，现在是批量编码)，全为0，表示只有一个句子，没有上下文
